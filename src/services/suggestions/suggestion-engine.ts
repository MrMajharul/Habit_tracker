...
export class RuleBasedSuggestionEngine implements SuggestionEngine {
  async generateSuggestions(
        ...
        items: [
          {
            ...
            icon: "📖",
            category: "worship",
            title: "Qur'an Recitation (2 Juz/Pages)",
            durationMinutes: 20,
            actionUrl: "/quran",
            actionLabel: "Recite",
          },
    ...
    // Case 3: Prayer window with 30-60 minutes remaining (e.g. before Asr or Dhuhr)
    if (minutesLeft >= 30 && minutesLeft <= 65) {
      const topTask = context.pendingTasks?.[0];
      const taskTitle = topTask?.title ?? "Compiler Design Study";
      const studyDuration = Math.min(30, Math.max(20, minutesLeft - 15));
